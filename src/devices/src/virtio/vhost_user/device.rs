...
use nix::sys::socket::{recvmsg, ControlMessage, ControlMessageOwned, MsgFlags, sendmsg};
...
impl VhostUserDevice {
    ...
    fn handle_gpu_message_with_fds(
        ...
        match request {
            ...
            GpuBackendReq::DMABUF_SCANOUT | GpuBackendReq::DMABUF_SCANOUT2 => self.handle_gpu_dmabuf_scanout(request, payload, fds),
    ...
    fn handle_gpu_dmabuf_scanout(&mut self, request: GpuBackendReq, payload: &[u8], fds: Vec<RawFd>) {
        ...
        if fds.is_empty() {
            debug!("{}: disabling DMABUF scanout {scanout_id} (no FD)", self.device_name);
            if let Some(display) = self.get_display_instance()
                && let Err(e) = display.disable_scanout(scanout_id)
            {
                error!("{}: disable_scanout failed: {e}", self.device_name);
        ...
        if scanout.width == 0 || scanout.height == 0 {
            debug!("{}: disabling DMABUF scanout {scanout_id}", self.device_name);
            if let Some(display) = self.get_display_instance()
                && let Err(e) = display.disable_scanout(scanout_id)
            {
                error!("{}: disable_scanout failed: {e}", self.device_name);
        ...
        debug!(
            "{}: DMABUF_SCANOUT{} scanout_id={} {}x{} fourcc={:#x} modifier={:#x}",
            self.device_name,
            if request == GpuBackendReq::DMABUF_SCANOUT2 { "2" } else { "" },
            scanout_id,
            scanout.fd_width,
            scanout.fd_height,
            scanout.fd_drm_fourcc,
            modifier
        );

        let display_width = self.gpu_display_info.as_ref().map_or(scanout.width, |d| d.width);
        let display_height = self.gpu_display_info.as_ref().map_or(scanout.height, |d| d.height);
    ...
    fn handle_gpu_dmabuf_update(&mut self, payload: &[u8], request: GpuBackendReq) {
        ...
        debug!(
            "{}: DMABUF_UPDATE scanout_id={} rect=({},{} {}x{})",
            self.device_name, scanout_id, rect.x, rect.y, rect.width, rect.height
        );
