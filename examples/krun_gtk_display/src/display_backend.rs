...
use std::mem;
use std::ptr::{null, null_mut, from_ref};
...
impl DisplayBackendDmabuf for GtkDisplayBackend {
    ...
    fn configure_scanout_dmabuf(
        ...
        // Just store which DMABUF is configured for this scanout
        // Don't build texture yet - that happens in present_dmabuf!
        self.channel
    ...
    fn present_dmabuf(
        ...
        // Use sync_channel with buffer=0 to block until GTK receives and processes
        ...
        // Block until texture build completes (NOT until paint!)
        // This is fast (~1-2ms) and provides correct synchronization
    ...
    fn into_display_backend(
        ...
        extern "C" fn create_fn(
            ...
            unsafe {
                ...
                let userdata_ref = (userdata as *const PollableChannelSender<DisplayEvent>).as_ref();
        ...
        extern "C" fn disable_scanout_fn(instance: *mut c_void, scanout_id: u32) -> i32 {
            from_rust_result(DisplayBackendBasicFramebuffer::disable_scanout(&mut *cast_instance(instance), scanout_id))
        }
        ...
        extern "C" fn import_dmabuf_fn(
            ...
            let dmabuf = unsafe { ptr_to_option_ref(dmabuf_export) }.expect("dmabuf_export must not be null");
        ...
        DisplayBackend {
            ...
            #[cfg(target_os = "linux")]
            features: (DisplayFeatures::BASIC_FRAMEBUFFER | DisplayFeatures::DMABUF_CONSUMER).bits(),
