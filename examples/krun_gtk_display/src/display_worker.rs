        ...
        while let Some(msg) = self.rx.try_recv().unwrap() {
            match msg {
                ...
                DisplayEvent::ConfigureScanoutDmabuf {
                    ...
                    debug!("Configure DMABUF scanout {scanout_id} with dmabuf ID {dmabuf_id}");
                    let dmabuf = self
                        .imported_dmabufs
                        .borrow()
                        .get(&dmabuf_id)
                    ...
                    if let Some(scanout) = &mut scanouts[scanout_id as usize] {
                        // **FIX: Unref old DMABUF before switching to new one**
                        if let Some(old_dmabuf_id) = scanout.get_current_dmabuf_id() {
                            if old_dmabuf_id != dmabuf_id {
                                debug!("Unreferencing old dmabuf ID {old_dmabuf_id}");
                                self.imported_dmabufs.borrow_mut().remove(&old_dmabuf_id);
                            }
                        }
                        // Just store which DMABUF is configured - don't build texture yet!
                        debug!("ConfigureScanoutDmabuf: storing dmabuf_id {} (texture build deferred to present)", dmabuf_id);
                        scanout.set_current_dmabuf_id(dmabuf_id);
                    } else {
                        warn!("Attempted to configure DMABUF on non-existent scanout: {scanout_id}");
                    }
                }
                #[cfg(target_os = "linux")]
                DisplayEvent::UpdateScanoutDmabuf {
                    ...
                    if let Some(scanout) = &mut scanouts[scanout_id as usize] {
                        if let Some(dmabuf_id) = scanout.get_current_dmabuf_id() {
                            debug!("UpdateScanoutDmabuf: calling set_dmabuf for dmabuf_id {}", dmabuf_id);
                            let dmabuf = self
                                .imported_dmabufs
                                .borrow()
                                .get(&dmabuf_id)
                            ...
                            // Build texture immediately and send response
